...
cdef class UpdraftVariables:
    def __init__(self, nu, namelist, paramlist, Grid.Grid Gr):
        ...
        self.upd_cloud_base = np.zeros((nu,), dtype=np.double, order='c')
        self.upd_cloud_top = np.zeros((nu,), dtype=np.double, order='c')
        self.upd_cloud_cover = np.zeros((nu,), dtype=np.double, order='c')
        self.upd_lwp = 0.
        self.upd_cloud_fraction = np.zeros((nzg,), dtype=np.double, order='c')
        return
        ...
        with nogil:
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                ...
                if self.QL.bulkvalues[k] > 1e-8 and self.Area.bulkvalues[k] > 1e-3:
                    self.upd_cloud_fraction[k] = self.Area.bulkvalues[k]
                else:
                    self.upd_cloud_fraction[k] = 0.
        return
    ...
    cpdef io(self, NetCDFIO_Stats Stats, ReferenceState.ReferenceState Ref):
        ...
        Stats.write_profile('updraft_cloud_fraction', self.upd_cloud_fraction[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        ...
        Stats.write_ts('updraft_cloud_cover', np.sum(self.upd_cloud_cover))
        Stats.write_ts('updraft_cloud_base',  np.amin(self.upd_cloud_base))
        Stats.write_ts('updraft_cloud_top',   np.amax(self.upd_cloud_top))
        Stats.write_ts('updraft_lwp',         self.upd_lwp)
        return
    ...
    cpdef upd_cloud_diagnostics(self, ReferenceState.ReferenceState Ref):
        ...
        for i in xrange(self.n_updrafts):
            ...
            self.upd_cloud_base[i]  = self.Gr.z_half[self.Gr.nzg - self.Gr.gw - 1]
            self.upd_cloud_top[i]   = 0.0
            self.upd_cloud_cover[i] = 0.0

            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                self.upd_lwp += Ref.rho0_half[k] * self.QL.values[i,k] * self.Area.values[i,k] * self.Gr.dz

                if self.QL.values[i,k] > 1e-8 and self.Area.values[i,k] > 1e-3:
                    self.upd_cloud_base[i]  = fmin(self.upd_cloud_base[i], self.Gr.z_half[k])
                    self.upd_cloud_top[i]   = fmax(self.upd_cloud_top[i], self.Gr.z_half[k])
                    self.upd_cloud_cover[i] = fmax(self.upd_cloud_cover[i], self.Area.values[i,k])
        return
