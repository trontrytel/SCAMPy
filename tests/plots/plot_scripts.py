...
def plot_humidities(data, les,tmin, tmax, title,  folder="plots/output/"):
    ...
    mpl.rc('lines', linewidth=4, markersize=10)
    plt.subplot(3,3,1)
    plt.plot(np.nanmean(les["qv_mean"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["qv_mean"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("mean qv [g/kg]")
    plt.ylabel("z [km]")
    plt.grid(True)
    plt.subplot(3,3,2)
    plt.plot(np.nanmean(les["upd_qv"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["upd_qv"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("updraft qv [g/kg]")
    plt.grid(True)
    plt.subplot(3,3,3)
    plt.plot(np.nanmean(les["env_qv"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["env_qv"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("env qv [g/kg]")
    plt.grid(True)

    plt.subplot(3,3,4)
    plt.plot(np.nanmean(les["ql_mean"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["ql_mean"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("mean ql [g/kg]")
    plt.ylabel("z [km]")
    plt.grid(True)
    plt.subplot(3,3,5)
    plt.plot(np.nanmean(les["updraft_ql"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["updraft_ql"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("updraft ql [g/kg]")
    plt.grid(True)
    plt.subplot(3,3,6)
    plt.plot(np.nanmean(les["env_ql"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["env_ql"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("env ql [g/kg]")
    plt.grid(True)

    plt.subplot(3,3,7)
    plt.plot(np.nanmean(les["qr_mean"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["qr_mean"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("mean qr [g/kg]")
    plt.ylabel("z [km]")
    plt.grid(True)
    plt.subplot(3,3,8)
    plt.plot(np.nanmean(les["updraft_qr"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["updraft_qr"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
    plt.xlabel("updraft qr [g/kg]")
    plt.grid(True)
    plt.subplot(3,3,9)
    plt.plot(np.nanmean(les["env_qr"][:,t_start_les:t_end_les],axis=1), les["z_half"], '-', color='gray', label='les', linewidth = 2)
    plt.plot(np.nanmean(data["env_qr"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue", label='les', linewidth = 4)
...
def plot_timeseries_1D(data,  les, folder="plots/output/"):
    ...
    # From Yair        ^                                                                                            ^                          ^
    #those are actually mean values not updraft only
